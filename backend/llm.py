...
def build_system_prompt(character: dict, state: dict) -> str:
    ...
    # 设备叙事：触手类角色本体即设备；装置类角色通过遥控/机械支配（由角色配置 device_narrative 决定）
    if str(character.get("device_narrative") or "触手") == "触手":
        narrative = (
    ...
    # 怒气值：画面持续黑暗 / 麦克风持续无声 → 逐轮暴怒
    rage = int(state.get("rage_rounds") or 0)
    if rage >= 5:
        lines.append(
