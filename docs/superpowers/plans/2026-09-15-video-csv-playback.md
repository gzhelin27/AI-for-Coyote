...
Tasks 1–7 are implemented and independently reviewed. The concrete adapter is
`GameLoopVideoOutput`, ramp proposals use confirmed receipts, and session tests
use the actual SafetyManager/GameLoop with synthetic transport rather than a
second fake execution model. These interface refinements preserve this plan's
requirements. API/storage operations and frontend state are kept in independent
video modules; only minimal main/App/GameLoop integration deltas touch inherited
dirty files. Browser acceptance uses the actual MP4 decoder and synthetic source.

Task 8: 75 focused video tests (1 platform skip), frontend 54 tests, clean install,
production build, compileall and two real Chrome dry-run scripts have passed.
The full regression rerun passed: 1,198 tests in 194.422s, 7 platform
skips, exit 0. Three pre-existing test timing origins were corrected without
widening their response thresholds. See
`docs/video-verification.md` for evidence and final counts.
Real-device acceptance/deployment/release tag remain deliberately pending.
...
Latest user authorization (2026-09-15, conveyed by the mother task): proceed autonomously with isolated video implementation, automated tests and synthetic dry-run validation. Previous novel-mode real-device acceptance is still pending and is not claimed passed. It does not block this explicitly authorized offline development; real-device acceptance, release tags and deployment remain separate gates.
...
- [x] Write the failing interval test and malformed-input table.
...
- [x] Run `& $videoPython -m unittest tests.test_video_csv -v`; verify failure from missing implementation.
- [x] Implement strict time parsing with `re.fullmatch(r'[0-9]+:[0-5][0-9]:[0-5][0-9]', text)`, strict four-column CSV decoding with UTF-8 BOM support, integer targets 0–200, and sorted non-overlapping bounds. Check video duration is finite, positive integer milliseconds before parsing. Reject rows beyond it. Use binary search and check `start <= position < end`; negative/out-of-video positions return no interval.
- [x] Test BOM, unsorted rows, adjacent boundaries, duplicate/overlapping ranges, malformed time/targets, too many rows, oversized bytes, missing columns, zero/negative duration, and output-zero gaps.
- [x] Run focused tests, then commit only these files as `feat: parse video intensity timelines`.
...
- [x] Write tests for 70-second partitioning and seed stability.
...
- [x] Write the user-confirmed cases before implementation.
...
- [x] Run `& $videoPython -m unittest tests.test_video_ramp -v`; verify red.
- [x] Implement target clipping, immediate decreases, no-op equal strength, fresh target allowed-step proposal, and ongoing ramp `confirmed + 1` only when `now_s >= last_increase_s + 2`. Store the original request so unchanged target messages cannot restart an initial jump. For changed target during an active ramp: immediate when the new difference fits max_step; otherwise retain ramp cadence. If the cap decreases below confirmed strength propose the lower cap immediately. Never infer confirmation from proposed values.
- [x] Test 20→31, 0→30 through 40 real seconds, 30→20, target 60 capped at 40, cap reduction, same target with remaining difference 10, changed target during ramp, failed proposal without confirmation, repeated timestamps, long scheduler delay and cancel/resume. Commit `feat: track conditional video strength ramps`.
...
- [x] Write fake-relay tests using the existing real GameLoop test harness.
...
- [x] Test no positive frames after stale generation, caps/max step, partial raw cycle at boundary, remaining 5 seconds after a seek, late ACK, helper cleanup, failed clear, estop while waiting and zero channel. Run existing manual-next and relay-ACK tests. Commit `feat: execute bounded video output through safety`.
...
- [x] Write synthetic clock and output-port fixtures. Fake output keeps a receipt queue, records calls, blocks individual ACKs on asyncio.Event and exposes a manual real-time clock. Test the state machine without network or media decoding.
...
- [x] Write endpoint tests using in-process HTTP and temporary stores, with synthetic bytes and fake decoded duration. AppState test fixtures must supply every new dependency explicitly, including existing `story_import_directory`.
...
- [x] Write clock bridge tests before UI integration.
...
- [x] Build an in-process synthetic integration fixture with real SafetyManager/GameLoop, fake relay, video session, temporary source store, fake monotonic time and deterministic seed. Assert that dry-run submits zero real relay frames.
...
- [x] Run all video unit/integration tests, existing manual-next/relay-ACK/coordinator/novel tests, and frontend browser tests. Use 30/30/10 blocks, zero gaps, all ramp examples, slow/failed ACKs, safety cap changes, expired leases and repeated seeks.
- [x] Run full Python unittest discovery, compileall, frontend clean install and production build. Do not invoke `tests/probe_llm.py`. Repair baseline fixture errors in their own reviewed commit before declaring the suite green; compare failures to the documented baseline rather than hiding them.
...
- [x] Perform code review focused on safety preemption, stale ownership, partial ACK and row-boundary behavior. Fix important findings and repeat only affected checks plus required gates.
- [x] Deliver the isolated branch and verification evidence to the user. Push only the user's origin fork if delivery requires it. Do not deploy, start output, create a release tag or mark the video phase accepted until the user completes its real-device gate.
...
The plan intentionally leaves the running mother checkout untouched. The user now authorizes code implementation and offline verification in this isolated worktree. This does not waive real-device acceptance or authorize deployment.
