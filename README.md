...
时间线、重放、手动控制和安全降档共用同一条设备输出协调路径。页面显示的设备状态分为两层：**期望状态**是当前策略希望设备达到的上限、通道开关和输出；**已确认状态**只在中继实际确认发送后更新。发生发送失败、断线或取消时，系统保留上一次已确认状态，不会把期望状态当作设备已经执行。

- 当运行时上限降低、设备报告过热、关闭通道、暂停、断线或急停需要降低/清除输出时，系统先停止受影响的时间线运行器，再执行物理降档或清除。未确认的工作会显示为待安全协调；下一次相同设备报告或安全协调操作会重试同一个目标。
...
- `dry_run: true` 使用同一协调、仲裁、会话归档和重放流程，但不会向真实中继或设备发送帧，可用于验证这些失败与恢复路径。精确重放不会重新抽取随机数；应用或 DLC 来源指纹变化会把重放标记为「已调整」。
